...
namespace TRGE.Core.Test
{
    ...
    public class AudioTests : BaseTestCollection
    {
        ...
        private readonly string _localWavFolder = string.Empty;
        ...
        //[TestMethod]
        //[TestSequence(0)]
        protected void BuildTR2Audio()
        {
            Dictionary<int, Tuple<ushort, string>> mp3Map = new Dictionary<int, Tuple<ushort, string>>
            {
            ...
            /*using (BinaryWriter bw = new BinaryWriter(new FileStream("tr2audio.wad", FileMode.Create, FileAccess.Write)))
            {
                ...
                uint offset = 268 * Convert.ToUInt32(count) + sizeof(ushort);
                bw.Write(count);
                foreach (int key in mp3Map.Keys)
                {
                    bw.Write(mp3Map[key].Item1);
                    byte[] description = Encoding.ASCII.GetBytes(mp3Map[key].Item2);
                    Array.Resize(ref description, 258);
        ...
        //[TestMethod]
        protected void BuildTR3Audio()
        {
            List<TRAudioTrack> tracks = new List<TRAudioTrack>();
            string dir = Path.Combine(_localWavFolder, "TR3Audio");
            /*using (BinaryReader br = new BinaryReader(new FileStream(@"audio\tr3audio.wad", FileMode.Open)))
            {
            ...
            Dictionary<ushort, string> tr3Map = new Dictionary<ushort, string>
            {
            ...
            using (BinaryWriter bw = new BinaryWriter(new FileStream("tr3audio.dat", FileMode.Create, FileAccess.Write)))
            {
                ...
                uint offset = 0;// 268 * Convert.ToUInt32(count) + sizeof(ushort);
                bw.Write(count);
                ...
                {
                    bw.Write(key);
                    byte[] description = Encoding.ASCII.GetBytes(tr3Map[key]);
                    Array.Resize(ref description, 258);
                    bw.Write(description);
                    uint length = Convert.ToUInt32(new FileInfo(Path.Combine(dir, key + ".wav")).Length);
                    bw.Write(length);
        ...
        protected void TestTR2Audio()
        {
            ...
            try
            {
                Assert.IsTrue(sm.LevelManager.AudioProvider is TR2AudioProvider);
                TRAudioTrack track = sm.TitleTrack;
                Assert.IsNotNull(track);
                Assert.AreEqual(track.Name, "Main Theme");
                /*byte[] data = provider.GetTrackData(track);
        ...
        protected void TestTR3Audio()
        {
            ...
            try
            {
                Assert.IsTrue(sm.LevelManager.AudioProvider is TR3AudioProvider);
                TRAudioTrack track = sm.TitleTrack;
                Assert.IsNotNull(track);
                Assert.AreEqual(track.Name, "Lara's Themes");
                /*byte[] data = provider.GetTrackData(track);
                using (MemoryStream ms = new MemoryStream(data))
                {
                    new SoundPlayer(ms).Play();
